...
export class DocumentService {
  ...
  uploadFile = async (
    ...
    try {
      ...
      onProgress?.(95);
      const fileDetails = await this.getFile(
        confirmResponse.file_id,
        targetProjectId
      );
  ...
  getAvailableParsingServices = async (): Promise<
    Array<{
      name: string;
      status: string;
      capabilities: string[];
      ...
      const parsedDocuments = await Promise.all(
        files.map(async (file, index) => {
          ...
          try {
            ...
            console.error(
              `Failed to parse document ${file.original_name}:`,
              parseError
            );
            throw new Error(
              `Failed to parse document "${file.original_name}": ${
  ...
  private analyzeDocuments = (
    ...
    // Generate insights
    const { keyDifferences, commonElements } = this.generateInsights(
      parsedDocuments,
      files
    );
    ...
    return {
      ...
      summary: `The documents show ${
        overallSimilarity > 70 ? "high" : "moderate"
      } similarity with consistent ${
        structureMatch > 0.8 ? "structure and formatting" : "content themes"
      }.`,
    };
  };
  ...
  private generateInsights = (
    ...
    if (files.length > 2) {
      keyDifferences.push(
        "Multiple document versions with incremental changes"
      );
    }

    const totalBlocks = documents.reduce(
      (sum, doc) => sum + doc.content_blocks.length,
      0
    );
    if (totalBlocks > 50) {
