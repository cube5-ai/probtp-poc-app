...
**Key Principles**:
1. **Find the best match**: Identify the {vendor_a_ref_name} level amongst {vendor_a_ref_levels} that is globally equivalent or better than {vendor_b_name}.
2. **Identify winners**: Given the identified best matching level, identify the wins for {vendor_a_ref_name}, the wins for {vendor_b_name} and the draws.
3. **Use world knowledge**: Assess real-world value of differences
   - Example: If {vendor_b_name} covers up to 900€ but in general average cost of the prestation is 250€, and {vendor_a_ref_name} covers 300€, the gap is insignificant as {vendor_a_ref_name} is already covering more than the average cost. So the {vendor_b_name} coverage might be marketing fluff.
   - Example: If {vendor_a_ref_name} is better on Prestation A (used frequently) but worse on Prestation B (rarely used), emphasize frequency
4. **Be balanced**: Acknowledge {vendor_b_name}'s wins but explain why they matter less
5. **Focus on customer value**: Pick examples the decision-maker would care about most
