...
def parse_document(file_path: str):
    """
    Parse with Extend, then repair per page with Gemini using PyMuPDF for exact wording.
    """
        ...
        try:
            answer_resp = client.models.generate_content(
                ...
                config=GenerateContentConfig(
                    thinking_config=ThinkingConfig(thinking_budget=10_000)
                ),
            )
            improved_markdown = getattr(answer_resp, "text", "").strip() or improved_markdown
        except Exception:
            improved_markdown = extend_md or pymupdf_md
