...
MANDATORY_SANITY_CHECKS = [
    SanityCheck(
        ...
        description=(
            ...
            "(2) Headers should be unique - repeated header names indicate missing headers; "
...
async def process_document(file_path: str, client: genai.Client) -> dict:
    ...
    total_landing_tables = sum(len(tables) for tables in landing_tables_by_page.values())
    total_pymupdf_tables = sum(len(tables) for tables in pymupdf_tables_by_page.values())
    print(f"  Landing AI: {total_landing_tables} tables across {len(landing_tables_by_page)} pages")
    print(f"  PyMuPDF: {total_pymupdf_tables} tables across {len(pymupdf_tables_by_page)} pages")
    ...
    for page_num in landing_tables_by_page:
        ...
        for landing_idx, table in enumerate(landing_tables):
            matched_indices = matches.get(landing_idx, [])
            matched_pymupdf = [pymupdf_tables[i] for i in matched_indices if i < len(pymupdf_tables)]
            ...
            corrected_tables.append((landing_idx, corrected_table))
        ...
        # Sort corrected tables by landing index
        corrected_tables_by_page[page_num].sort(key=lambda x: x[0])
    ...
    for page_num in corrected_tables_by_page:
        for landing_idx, table in corrected_tables_by_page[page_num]:
            table_idx = landing_idx
            header_result = await run_sanity_checks(client, table, MANDATORY_SANITY_CHECKS)
            ...
            if table['has_valid_headers']:
                print(f"    ✓ Page {page_num}, Table {table_idx}: Valid headers")
            else:
                print(f"    ✗ Page {page_num}, Table {table_idx}: Missing/invalid headers")
    
    # Second pass: process violations and apply corrections (only mandatory checks)
    ...
    for page_num in corrected_tables_by_page:
        ...
        for landing_idx, table in corrected_tables_by_page[page_num]:
            ...
            # Extract table ID for debug
            import re
            table_id_match = re.search(r'<table id="([^"]+)"', table.get('html_content', ''))
            table_id = table_id_match.group(1) if table_id_match else 'unknown'
            pdf_name = Path(file_path).stem
            
            # Get matched PyMuPDF tables for debug artifacts
            matched_indices = matches_by_page.get(page_num, {}).get(landing_idx, [])
            matched_pymupdf = [pymupdf_tables_by_page.get(page_num, [])[i] 
            ...
            if violations:
                ...
                print(f"    Page {page_num}, Table {table_idx}: {len(violations)} violations found")
                
                # Find previous table with valid headers (closest one before current table)
                previous_table = None
                
                # Look through all tables before current one (same page)
                for prev_idx, prev_table_candidate in corrected_tables_by_page[page_num]:
                    # Only consider tables that come before the current table
                    if prev_idx >= landing_idx:
                        break
                    # Keep the last valid one we find (closest to current)
                    if prev_table_candidate.get('has_valid_headers', False):
                        previous_table = prev_table_candidate
                
                # If not found on same page, look at previous pages (starting from most recent)
                if not previous_table:
                    for prev_page in range(page_num - 1, -1, -1):
                        if prev_page not in corrected_tables_by_page:
                            continue
                        # Check all tables on this page (in reverse to get the last one)
                        for prev_idx, prev_table_candidate in reversed(corrected_tables_by_page[prev_page]):
                            if prev_table_candidate.get('has_valid_headers', False):
                                previous_table = prev_table_candidate
                                break
                        if previous_table:
                ...
                if previous_table:
                    # Extract reference table ID for debugging
                    ref_table_id_match = re.search(r'<table id="([^"]+)"', previous_table.get('html_content', ''))
                    ref_table_id = ref_table_id_match.group(1) if ref_table_id_match else 'unknown'
                    print(f"      → Found reference table {ref_table_id} with valid headers for context")
                else:
                    print(f"      → No reference table with valid headers found")
                
                # Generate screenshot and apply visual correction
                try:
                    ...
                    visual_corrections = corrected_table.get('visual_corrections_applied', 0)
                    ...
                    # Save HTML versions
                    # save_html_versions(corrected_table, table_id, page_num, pdf_name)
                ...
                save_html_versions(table, table_id, page_num, pdf_name)
                final_tables.append(table)
...
async def main():
    ...
    async with AsyncGeminiClientManager() as client:
        ...
        json_file.write_text(json.dumps(serializable_result, indent=2), encoding='utf-8')
